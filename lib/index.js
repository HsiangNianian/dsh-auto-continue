...
var AutoContinueRunner = class {
  ...
  chunkTextFromParts(value) {
    if (typeof value === "string") return value;
    if (!Array.isArray(value)) return "";
    return value.filter((part) => typeof part === "object" && part !== null && part.type === "text" && typeof part.text === "string").map((part) => part.text).join("");
  }
  assistantChunkText(event) {
    ...
    const fromDelta = this.chunkTextFromParts(record["delta"]?.content);
    const fromChunk = this.chunkTextFromParts(record["chunk"]?.content);
    const fromMessage = this.chunkTextFromParts(record["messageDelta"]?.content);
  ...
  withinEditDistance(left, right, maxDistance) {
    ...
    for (let row = 1; row <= leftLen; row += 1) {
      const current = new Array(rightLen + 1);
      current[0] = row;
      let minInRow = row;
      for (let col = 1; col <= rightLen; col += 1) {
        ...
        const substitution = (previous[col - 1] ?? Number.MAX_SAFE_INTEGER) + (left.charCodeAt(row - 1) === right.charCodeAt(col - 1) ? 0 : 1);
