...
- **无输出回合续跑** — 模型有时只输出一段推理就结束回合: 没有文本, 也没有工具调用。DSH 把它记为正常结束(`completed`), 于是没有任何机制续跑, Agent 停下来等人手动输入「继续」。回合以 `completed` 结束且没有可见输出(没有非空文本、没有工具调用)时, 插件发送专用的续跑文本; 这种续跑不清零连续次数, 冷却与上限照常约束每轮都卡住的模型, 出现有可见输出的回合或用户消息后才清零。DSH 为同一种回合提议的 `no-visible-output` 结束原因按同样方式处理。可用「续跑无输出回合」开关关闭
